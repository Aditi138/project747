...
from models.tri_attn_ablation_model import TriAttn
		...
		for index, query_embed in enumerate(batch['q_embed']):
			...
			if args.sentence_scoring:
				...
				if args.reduced:
					...
					s_file.write(
						"@@".join(context_tokens_per_docid[doc_id]) + "\n" + " ".join(batch_q_tokens[index]) + "\n")
			...
			for index, query_embed in enumerate(batch['q_embed']):
				...
				if args.sentence_scoring:
					...
					batch_context_mask = variable(
						torch.FloatTensor(np.array([1 for x in range(batch_context_length[0])])))
...
if __name__ == "__main__":
	...
	if args.squad:
		...
		train_documents, train_candidates_embed_docid, train_candidate_per_docid, train_context_per_docid, train_context_tokens_per_docid, _ = loader.load_documents_elmo(
			t_documents, split=False, train=True)
		valid_documents, valid_candidates_embed_docid, valid_candidate_per_docid, valid_context_per_docid, valid_context_tokens_per_docid, _ = loader.load_documents_elmo(
			v_documents, split=False)
		test_documents, test_candidates_embed_docid, test_candidate_per_docid, test_context_per_docid, test_context_tokens_per_docid, _ = loader.load_documents_elmo(
			te_documents, split=False)
	...
	# exit(0)
		...
		train_documents, train_candidates_embed_docid, train_candidate_per_docid, train_context_per_docid, train_context_tokens_per_docid, train_context_ranges_per_docid = loader.load_documents_split_sentences(
			t_documents, train=True)
		print("Loading validation documents")
		valid_documents, valid_candidates_embed_docid, valid_candidate_per_docid, valid_context_per_docid, valid_context_tokens_per_docid, valid_context_ranges_per_docid = loader.load_documents_split_sentences(
			v_documents)
		print("Loading testing documents")
		test_documents, test_candidates_embed_docid, test_candidate_per_docid, test_context_per_docid, test_context_tokens_per_docid, test_context_ranges_per_docid = loader.load_documents_split_sentences(
			te_documents)
