...
from bidaf import BiDAF, LinearSeqAttn, weighted_avg, BiLinearAttn, MultiHeadBiLinearAttn, MultiHeadLinearSeqAttn
import codecs
...
class TriAttnMultiHead(nn.Module):
	...
	def forward(self, query_embedded, batch_query_length, batch_query_mask,
		...
		query_aware_context_encoded, _ = self.attention_flow_c2q(query_embedded, context_embedded, batch_query_mask,
																 batch_context_mask)
		...
		context_aware_answer_encoded, _ = self.attention_flow_a2c(
			context_embedded.expand(batch_size, context_embedded.size(1), context_embedded.size(2)),
			batch_candidates_embedded,
		...
		query_self_attention, query_input_modelled_transformed = self.self_attn_q(query_input_modelled, batch_query_mask)
		q_hidden = weighted_avg(query_input_modelled_transformed, query_self_attention)

		answer_self_attention, answer_modeled_transformed = self.self_attn_a(answer_modeled, batch_candidate_masks_sorted)
		a_hidden = weighted_avg(answer_modeled_transformed, answer_self_attention)

		context_self_attention, context_modeled_transformed = self.self_attn_c(context_modeled, q_hidden, batch_context_mask)
	...
	def eval(self, query_embedded, batch_query_length, batch_query_mask,
		...
		query_aware_context_encoded, c2q_attention_matrix = self.attention_flow_c2q(query_embedded, context_embedded,
																					batch_query_mask,
																					batch_context_mask)
		query_aware_context_encoded = self.dropout(query_aware_context_encoded)
