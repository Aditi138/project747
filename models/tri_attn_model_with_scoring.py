...
class TriAttn(nn.Module):
	...
	def eval(self,query_embedded, batch_query_length,batch_query_mask,
		...
		query_embedded_chunk_wise   = query_embedded.expand(num_chunks, query_embedded.size(0), query_embedded.size(1))
		...
		query_aware_context_encoded, c2q_attention_matrix = self.attention_flow_c2q(query_embedded_chunk_wise, context_embedded, batch_query_mask_chunk_wise,
																 batch_context_mask)
		query_aware_context_encoded = self.dropout(query_aware_context_encoded)
		...
		context_mask_combined = batch_context_mask.view(-1).unsqueeze(0)
		...
		context_aware_answer_encoded, _ = self.attention_flow_a2c(
			...
			batch_candidates_embedded,
			context_mask_combined.expand(batch_size, context_mask_combined.size(0),context_mask_combined.size(1)), batch_candidate_masks_sorted)
