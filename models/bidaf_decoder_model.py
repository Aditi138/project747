...
class BiDAFDecoder(nn.Module):
	def __init__(self, args, loader):
		super(BiDAFDecoder, self).__init__()
		...
		## modelling layer for question and context : this layer also converts the 8 dimensional input into two dimensional output
		...
		## decoder
		# modelling layer after c2q,:c used as attention, context used as vocab and teacher forcing
		self.lstm = nn.LSTM(embed_size, hidden_size,
							num_layers=2, bidirectional=False, dropout=args.dropout, batch_first=True)
	...
	def forward(self, query_embedded, batch_query_length, batch_query_mask,
				...
				batch_candidates_embedded, batch_candidate_lengths_sorted, batch_candidate_masks_sorted,
		...
		## hidden layer of decoder ## last question encoder + last c2c:c encoder hidden state through W

		## decoding layer
		max_answer_length = max(batch_candidate_lengths_sorted)
		loss_vector = variable(torch.zeros(max_answer_length, batch_size))
		lengths = variable(torch.FloatTensor(batch_candidate_lengths_sorted))  ## does this have to be a volatile element?
		for timestep in range(max_answer_length):
			## combining quest encoding + previous input encoding (teacher forcing)
			decoder_input_vector = torch.cat([decoder_embedding_input.unsqueeze(2), query_unsorted], dim=1).transpose(1,
																													  2).contiguous()

			## generate next hidden state
			decoder_output, lstm_hidden = self.lstm(decoder_input_vector, lstm_hidden)

			scores = torch.bmm(document_unsorted, decoder_output.permute(0, 2, 1).contiguous())
			probs = self.softmax_mask(scores, batch_document_mask)

			## batch answer location mask = batch size * max answer length * max document length
			## batch answers = batch_size * max_ answer length
			## answer length mask = batch size * max answer length
			answer_timestep_probs = torch.sum(probs.squeeze(2) * batch_answer_location_mask[:, timestep], 1) + 1e-20
			loss_vector[timestep] = -(torch.log(answer_timestep_probs)) * batch_answer_length_mask[:, timestep]
			decoder_embedding_input = self.embedding_layer_encoder(batch_answers[:, timestep])
		# decoder_embedding_input = self.embedding_layer_dropout(decoder_embedding_input)
		loss = torch.sum(loss_vector) / torch.sum(lengths)
